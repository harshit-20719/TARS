...
1. OBSERVATIONS. Work through the ${n} rows below **one at a time**. For each row, re-read the transcript asking a single question: what in here is evidence about this row? Then quote it.
   ...
   - Several quotes per row is normal and useful. A row supported by three passages is better evidence than a row supported by one. But stop when a row is covered: three or four quotes is a well-covered row, and past five you are quoting around the same point — take the best and move to the next row. A finished block is usually ten to twenty observations across all ${n} rows. If you find yourself well beyond that, you are repeating yourself; close the list and stop.
