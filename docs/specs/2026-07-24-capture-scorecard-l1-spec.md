...
**D6. Extraction model.** *Amended 2026-07-30.* The drafting step runs on Google Gemini — `gemini-2.5-flash-lite`, pinned exactly — through `@google/genai`, with the Anthropic path kept selectable by environment configuration. Changed for cost, roughly a twentieth per run, and because Biome holds Gemini credits; the port and its decisions live in `docs/plans/2026-07-30-005-feat-extraction-overhaul-plan.md` (KTD1, KTD2). It remains one setting and does not block the build. Superseded text: the drafting step uses a Claude model through the Anthropic SDK, defaulting to `claude-opus-4-8` in config with `EXTRACTION_MODEL=claude-sonnet-5` for the pilot, since this is a high-volume drafting step where the PM authors every score and Sonnet is the production-volume tier.
