...
- KTD3. **Extraction uses structured outputs (`client.messages.parse()` + a Zod schema), not tool-use.** Extraction returns data, not function calls; `messages.parse()` validates the model output against the schema automatically. The schema shapes observations (quote, rubric, sub-dimension, source, speaker/timestamp) and claim drafts (value, anchor, origin tag, status *claimed*). Governs R3, R4. *(Amended 2026-07-30: superseded by the provider port — extraction no longer goes through `messages.parse()`. The enum this decision relied on reached the wire as a prose hint, enforced only by the SDK's client-side parse; the port enforces it in the request schema and re-validates in the adapter. See `docs/plans/2026-07-30-005-feat-extraction-overhaul-plan.md` KTD4, KTD8.)*
- KTD4. **Extraction model is env-configurable (`EXTRACTION_MODEL`), default `claude-opus-4-8`; Sonnet 5 (`claude-sonnet-5`) is the recommended optimization.** The step is high-volume drafting where the machine never judges — Sonnet 5 is near-Opus on extraction at ~⅓ the cost and is a one-line config change. Default stays on the SDK's safe tier; the recommendation is surfaced for the user to confirm before pilot (see Open Questions). Governs R5. *(Amended 2026-07-30: the default provider is now Gemini — `gemini-2.5-flash-lite`, pinned — for cost, roughly a twentieth per run, and because Biome holds Gemini credits. `EXTRACTION_MODEL` remains the override and the Anthropic path stays selectable; the tier reasoning above still governs the Anthropic fallback. See `docs/plans/2026-07-30-005-feat-extraction-overhaul-plan.md` KTD1, KTD2.)*
